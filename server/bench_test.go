	...
	for i := 0; i < b.N; i++ {
		...
		r, err := client.R().
			SetBody(&data.UserPatch{
				Labels: []string{
					fmt.Sprintf("label%d001,updated %d times", userIndex, times),
					fmt.Sprintf("label%d002,updated %d times", userIndex, times),
				},
				Comment: proto.String(fmt.Sprintf("add label for user: %d, updated %d times", userIndex, times)),
			}).
...
func BenchmarkGetUser(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		userIndex := i % numInitUsers
		r, err := client.R().
			Get(s.address + "/api/user/init_user_" + strconv.Itoa(userIndex))
		require.NoError(b, err)
		require.Equal(b, http.StatusOK, r.StatusCode(), r.String())
	}
	b.StopTimer()
